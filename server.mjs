...
function renderQueue() { if (!requests.length) return '<p class="hint">The queue is ready for its first request.</p>'; return requests.filter(r => r.status === 'approved').slice(-5).map((r,i) => `<article class="queue-song" data-request-id="${escapeHtml(r.id)}"><span>0${i + 1}</span><div><b>${escapeHtml(r.title)}</b><small>${escapeHtml(r.artist)}</small></div><em>${escapeHtml(r.duration || '—')}</em></article>`).join('') || '<p class="hint">Requests will appear here once approved.</p>'; }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/requests') { if (!requireRole(req, res, 'guest') || !isSameOrigin(req)) return; const now=Date.now(), sid=parseCookies(req).wedding_session, recent=(requestLimits.get(sid)||[]).filter(t=>now-t<15*60*1000); if(recent.length>=3) return json(res,429,{error:'You can request up to 3 songs every 15 minutes.'}); const body=await readJson(req), song=songMetadata.get(body.id); if(!song || String(body.note||'').length>240) return json(res,400,{error:'Please choose a song from the search results.'}); const status = !song.explicit && song.durationSeconds > 0 && song.durationSeconds <= 360 ? 'approved' : 'pending', request={id:crypto.randomUUID(),videoId:song.id,title:song.title,artist:song.artist,duration:song.duration,note:String(body.note||''),status}; requests.push(request); recent.push(now);requestLimits.set(sid,recent);return json(res,201,{ok:true,status,request:status==='approved'?request:undefined,approvedCount:requests.filter(r=>r.status==='approved').length}); }
    if (req.method === 'GET' && url.pathname === '/api/host/player') { if (!requireRole(req,res,'host')) return; return json(res,200,{current:requests.find(r=>r.status==='playing')||null,next:requests.find(r=>r.status==='approved')||null,approvedCount:requests.filter(r=>r.status==='approved').length}); }
    if (req.method === 'POST' && url.pathname === '/api/host/request') { if (!requireRole(req,res,'host') || !isSameOrigin(req)) return; const body=await readJson(req), item=requests.find(r=>r.id===body.id); if(!item || !['approve','decline','start','complete'].includes(body.action)) return json(res,400,{error:'Invalid request.'}); if(body.action==='approve' && item.status==='pending') item.status='approved'; else if(body.action==='decline' && item.status==='pending') item.status='declined'; else if(body.action==='start' && item.status==='approved') item.status='playing'; else if(body.action==='complete' && item.status==='playing') item.status='played'; else if (!((body.action==='start' && item.status==='playing') || (body.action==='complete' && item.status==='played'))) return json(res,409,{error:'This request is no longer available for that action.'}); return json(res,200,{ok:true,item,approvedCount:requests.filter(r=>r.status==='approved').length}); }
